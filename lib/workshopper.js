...
function Workshopper (options) {
  ...
  if (argv._[0] == 'verify' || argv._[0] == 'run') {
    ...
    return this.execute(argv._[0], argv._[1], argv._.slice(2))
  }
...
Workshopper.prototype.fail = function (exercise) {
...
Workshopper.prototype.pass = function (exercise) {
...
Workshopper.prototype.execute = function (mode, submission, args) {
  ...
  exercise[mode](submission, args, function (err, pass) {
    if (err)
      return error('Could not process submission: ' + (err.message || err))

    if (mode == 'run')
      return // nothing to see here

    if (!pass)
      return this.fail(exercise)
